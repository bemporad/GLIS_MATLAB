...
if rbf_type == "inverse_quadratic"
    fun = @(x1, x2, epsil) 1/(1+epsil^2*sum((x1-x2).^2));
elseif rbf_type == "gaussian"
    fun = @(x1,x2,epsil) exp(-(epsil^2*sum((x1-x2).^2)));
elseif rbf_type == "multiquadric"
    fun = @(x1,x2,epsil) sqrt(1+epsil^2*sum((x1-x2).^2));
elseif rbf_type == "thin_plate_spline"
    fun = @(x1,x2,epsil) 2 * sum((x1-x2).^2) * log(epsil * sum((x1-x2).^2));
elseif rbf_type == "linear"
    fun = @(x1,x2,epsil) epsil * sqrt(sum((x1-x2).^2));
elseif rbf_type == "inverse_multi_quadric"
    fun = @(x1,x2,epsil) 1/sqrt(1+epsil^2*sum((x1-x2).^2));
else
